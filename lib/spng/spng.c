...
static int read_idat_bytes(spng_ctx *ctx, uint32_t *bytes_read) {
    ...
    if (ctx->streaming) {/* TODO: calculate bytes to read for progressive reads */
        len = SPNG_READ_SIZE;
    ...
    while (!(ret = read_header(ctx, &discard))) {
        ...
        if (is_critical_chunk(&chunk)) {
            if (!memcmp(chunk.type, type_iend, 4)) {
                ...
                return ret;
            } else if (!memcmp(chunk.type, type_idat, 4) && prev_was_idat) {/* ignore extra IDATs */
                ret = discard_chunk_bytes(ctx, chunk.length);
                if (ret) return ret;
...
static int
calculate_subimages(struct spng_subimage sub[7], size_t *widest_scanline, struct spng_ihdr *ihdr, unsigned channels) {
    ...
    int i;
    for (i = 0; i < 7; i++) {/* Calculate scanline width in bits, round up to the nearest byte */
...
#warning "zlib >= 1.2.11 is required for SPNG_CTX_IGNORE_ADLER32"
    ...
    if (ctx->ihdr.color_type == SPNG_COLOR_TYPE_GRAYSCALE_ALPHA ||
        ctx->ihdr.color_type == SPNG_COLOR_TYPE_TRUECOLOR_ALPHA)
    ...
    if (ctx->ihdr.bit_depth == 16 && fmt == SPNG_FMT_RGBA8) {/* in this case samples are scaled down by 8bits */
        sb.red_bits -= 8;
        sb.green_bits -= 8;
        sb.blue_bits -= 8;
        sb.alpha_bits -= 8;
    ...
    for (pass = 0; pass < 7; pass++) {
        ...
        for (scanline_idx = 0; scanline_idx < sub[pass].height; scanline_idx++) {
            ...
            } else {/* avoid creating an invalid reference */
                if (scanline_idx != (sub[pass].height - 1)) {
